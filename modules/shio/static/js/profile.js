document.addEventListener('DOMContentLoaded', () => {
    ...
    // Back button
    document.getElementById('back-to-select').addEventListener('click', () => switchView('shioList'));

    // Element name mapping for alter ego
    const elementNames = { kayu: 'Kayu 🌳', api: 'Api 🔥', tanah: 'Tanah ⛰️', logam: 'Logam ⚙️', air: 'Air 💧' };
    ...
    function fetchProfile(shioKey) {
        ...
        .then(data => {
            ...
            document.getElementById('p-branch').textContent = data.earthly_branch || '';
            document.getElementById('p-yinyang').textContent = data.yin_yang || '';
            document.getElementById('p-element').textContent = `Elemen: ${data.fixed_element || ''}`;
            ...
            const luckyItems = [
                ...
                { label: 'Warna Sial', value: (data.unlucky_colors || []).join(', ') },
                { label: 'Angka Hoki', value: (data.lucky_numbers || []).join(', ') },
                { label: 'Angka Sial', value: (data.unlucky_numbers || []).join(', ') },
                { label: 'Bunga Hoki', value: (data.lucky_flowers || []).join(', ') },
                { label: 'Bulan Terbaik', value: (data.best_months || []).join(', ') },
                { label: 'Bulan Terburuk', value: (data.worst_months || []).join(', ') }
            ];
            ...
            // Health
            document.getElementById('p-health').textContent = data.health_warning || '';

            // Spirit
            document.getElementById('p-spirit').textContent = data.spirit_advice || '';
