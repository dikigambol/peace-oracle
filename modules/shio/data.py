...
from .bank import (
    ...
    SHIO_DAILY_TIPS, SHIO_GUARDIAN_BANK, SHIO_PROFILE_BANK,
    SHIO_COMPATIBILITY_BANK, SHIO_FORTUNE_BANK
)
...
def get_shio_profile(shio_key):
    """Retrieve rich profile data for a given shio from the profile bank."""
...
def get_shio_compatibility(shio1_key, shio2_key):
    """Retrieve compatibility data for a pair of shio from the bank."""
    ...
    # Try both key orderings — bank uses alphabetically sorted tuple
    pair_key = tuple(sorted([shio1_key, shio2_key]))
    compat = SHIO_COMPATIBILITY_BANK.get(pair_key)
    
    # Also try unsorted in case bank uses different ordering
    if not compat:
...
def get_shio_fortune(shio_key):
    """Generate fortune reading for a shio using the fortune bank.
    
    Picks one random reading per category (karir, keuangan, asmara, kesehatan)
    from the fortune bank, using a daily seed so readings are consistent
    within the same day but change daily.
    """
    shio_data = SHIO_DATA.get(shio_key)
    if not shio_data:
        return {"error": f"Shio '{shio_key}' tidak ditemukan."}
    ...
    # Daily seed — consistent within same day, changes daily
